...
functionsList.toDbFromClient = (schemarow, key, value) => {
	...
	if (typeof value != "string") {
		if (schemarow.type == "int") {
			if (!functionsList.checkInt(value)) {
				return new Error(
					" Every value in a JSON body should be a STRING and can sometimes be an INT. In this case it has to be an INT. (is now probably a function or object)"
				);
			}
		} else {
			return new Error(
				" This value in the JSON body should be a string. This is maybe an object or something else."
			);
		}
	}
	...
	if (schemarow.encrypt == "1") {
		// Encrypt > AES encrypt. (searchable for get requests) and decryption possible
		var c = crypto.createCipheriv(
			"aes-128-ecb",
			functionsList.convertCryptKey("" + keys.encryptionKey + ""),
			""
		);
		var crypted = c.update(value, "utf8", "hex") + c.final("hex");
		value = crypted;
	}
	if (schemarow.encrypt == "2") {
		// Encrypt > Cryptr. not searchable value for GET but stronger encryption. Decryption possible
		value = cryptr.encrypt(value);
	}
	if (schemarow.encrypt == "3") {
		// Encrypt > Bcrypt. For passwords. no decryption possible not searchable.
